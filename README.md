...
### 📊 帳票出力機能
- **PDF/Excel形式の帳票生成**: 書籍データの統計レポート
- **リアルタイムプレビュー**: 生成された帳票の即時確認
- **高度なフィルタリング**: 読書状況・出版社・著者・ジャンル・期間での絞り込み
- **キャッシュ最適化**: 同一条件のレポートの高速生成
- **帳票テンプレート管理**: カスタマイズ可能なレポート形式
- **スケジュール配信**: 定期的な自動レポート生成と配信
- **権限管理**: ユーザーごとの帳票アクセス制御
- **履歴管理**: 生成済み帳票のダウンロードと再利用
...
-- - report_distributions (配信設定)
-- - report_permissions (権限設定)
-- - report_logs (帳票ログ)
-- - report_cache (キャッシュ)
```
...
### 📋 帳票出力 API（認証必須）
- `POST /api/reports/generate-preview` - 帳票生成とプレビュー
- `GET /api/reports/preview-content/{reportId}` - プレビューコンテンツ取得
- `GET /api/reports/download/{reportId}` - 帳票ダウンロード
- `GET /api/reports/history` - 帳票履歴取得
- `POST /api/reports/statistics` - 統計情報取得
- `GET /api/reports/status/{reportId}` - 生成ステータス確認
- `POST /api/reports/schedules` - スケジュール設定（管理者のみ）
- `GET /api/reports/templates` - テンプレート管理（管理者のみ）
- `POST /api/reports/cache/clear` - キャッシュクリア（管理者のみ）
...
### 帳票出力
1. **帳票出力画面**にアクセス
2. **フィルター設定**で出力対象データを絞り込み
   - 読書状況、出版社、著者、ジャンル、期間などで指定可能
3. **統計情報**で現在のフィルター条件での書籍数を確認
4. **PDF/Excel出力**ボタンで帳票を生成
5. **プレビュー画面**で内容を確認
6. **ダウンロード**または**履歴に保存**を選択
7. **生成履歴**から過去の帳票を再ダウンロード可能
8. **管理者**はシステム全体の統計帳票も出力可能
...
### 出力形式
- **PDF形式**: プレビュー対応、印刷用レイアウト
- **Excel形式**: データ編集・分析可能、グラフ対応

### 高度な機能
- **キャッシュ最適化**: 同一条件での高速生成
- **テンプレート管理**: カスタムレイアウト設定
- **スケジュール配信**: 定期的な自動生成・配信
- **権限管理**: レポートアクセス制御
- **履歴管理**: 過去の帳票の再ダウンロード
- **ログ記録**: 生成・ダウンロード履歴の追跡
...
- **帳票生成ログ**: `report_logs`テーブル
- **帳票履歴**: `report_history`テーブル
...
- `ReportService`: 帳票生成サービス
- `PDFReportService`: PDF生成サービス
- `ExcelReportService`: Excel生成サービス
- `ReportFileService`: ファイル管理サービス

#### フロントエンド
- `useBatch.ts`: バッチ処理カスタムフック
- `batchService.ts`: API呼び出しサービス層
- `Reports.tsx`: 帳票出力画面コンポーネント
...
- **📋 帳票出力機能**: PDF/Excel形式の統計レポート生成
- **🚀 キャッシュ最適化**: 高速な帳票再生成
- **📅 スケジュール配信**: 定期的な自動レポート生成
...
**蔵書管理システム v3.0** - エンタープライズレベルの機能とセキュリティ、高度な帳票出力機能を備えた最新の蔵書管理アプリケーション